...
def from_draw_to_angles_average(draws_folder, spin, length, sigma, burnin,
                                chain_id, angles_folder):
    ...
    if (len(path_collection) != 0):
        ...
        for draw_path in path_collection:
            ...
            angles_matrix = np.vectorize(from_intertwiner_to_angle)(
                angles_matrix, spin)
            ...
            df = pd.DataFrame(
                {
                    'angle average': [angles_matrix[0, 0]],
        ...
        warnings.warn(
            "I cannot combine chains since there are no chains available")
...
def funza(draw_path, tmp_angle_path):
    ...
    angles_matrix = np.matrix(df.values.transpose()).astype(float)
    angles_matrix = np.vectorize(from_intertwiner_to_angle)(angles_matrix,
                                                            spin)
    angles_matrix = np.matmul(angles_matrix, multeplicity)
    angles_matrix = angles_matrix.sum(axis=1) / (length - burnin)
    angles_matrix = angles_matrix.sum(axis=0) / 16
...
def from_draw_to_angles_average_multithreading(draws_folder, spin, length,
                                               sigma, burnin, angles_folder,
                                               number_of_threads):
    path_collection = []
    ...
    if (len(path_collection) != 0):
        ...
        threads = []
        for draw_path in path_collection:
            t = threading.Thread(target=funza,
                                 args=(
                                     draw_path,
                                     tmp_angle_path,
                                 ))
...
def retrieve_combined_angles(angles_folder, spin, length, sigma, burnin,
                             combined_chains):
...
def Metropolis_Hastings_parallel_run(draws_path, hash_tables_path, spin,
